    ...
    <div className="flex flex-col">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-700/80 bg-slate-800/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-slate-200">Launcher</h2>
          {getStateBadge()}
        </div>
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {!hasDefinition ? (
          <p className="text-xs text-slate-500 text-center py-2">
            Define a workflow in the Builder tab first
          </p>
        ) : (
          <>
            {/* Progress Bar */}
        ...
        {/* DAG Preview of defined workflow */}
        {hasDefinition && definition.workflow && (
          <div className="mt-2">
            <WorkflowDAG
              execution={null}
              preview
              previewTasks={definition.workflow.tasks.map((t: Record<string, unknown>) => ({
                name: String(t.name || ''),
                agent_id: String(t.agent_id || ''),
                requires_approval: t.requires_approval as boolean | undefined,
              }))}
              previewEdges={definition.workflow.edges}
            />
          </div>
        )}
      </div>

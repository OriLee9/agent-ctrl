...
> 一个从零构建的 Python Agent Workflow 框架，支持多 Agent 协作、DAG 工作流编排、Human-in-the-loop 审批、断点续跑、超长上下文文件管理。
...
│   ├── memory.py               # Conversation + Snapshot + Observer 模式
│   ├── file_context.py         # 超大文件管理（检索/分段读取/缓存）
│   ├── file_tools.py           # 文件操作 @tool 集合
│   ├── skill.py                # Skill 注册管理
...
│   ├── workflow.py             # 向后兼容别名
│   └── workflow/               # 合并后的工作流模块
│       ├── engine.py           # WorkflowEngine（DAG + V2 生产特性）
...
│   │   ├── MessageFlow.tsx     # 消息流可视化
│   │   ├── EventLog.tsx        # 实时事件日志
│   │   ├── InterventionPanel.tsx # 干预面板
│   │   └── WorkflowControl.tsx # Workflow 控制
...
│  ┌────────────────────▼────────────────────────────┐  │
│  │           FileContextManager                     │  │
│  │  search_code() | read_file_range() | list_files() │  │
│  └──────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
```bash
# 克隆项目后，安装 Python 依赖（仅 Flask 用于 API Server）
...
# Agent 获得 search_code / read_file_range / list_files / file_summary 能力
```
...
| POST | `/api/intervene`         | 提交干预        |
| GET  | `/api/snapshots`         | 快照列表        |
| POST | `/api/snapshot`          | 创建快照        |
| POST | `/api/rollback`          | 回滚快照        |
| GET  | `/api/workflow/tasks`    | Workflow 任务   |
| GET  | `/api/workflow/progress` | Workflow 进度   |
| POST | `/api/workflow/pause`    | 暂停            |
| POST | `/api/workflow/resume`   | 恢复            |
| POST | `/api/workflow/abort`    | 终止            |
